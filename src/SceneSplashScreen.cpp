...
void SceneSplashScreen::Render() {
    ...
    Vector2 raylib_logo_pos = { (float) round(window.GetRendererCenter().x - resources.GetTextureByName("raylib_logo").width / 2), (float) round(window.GetRendererCenter().y - resources.GetTextureByName("raylib_logo").height / 2) };
    ...
    int text_one_pos_x = window.GetRendererCenter().x - MeasureTextEx(GetFontDefault(), splashscreen_text_1.c_str(), 10, 1).x * 0.5f;
    int text_one_pos_y = window.GetRendererCenter().y - MeasureTextEx(GetFontDefault(), splashscreen_text_1.c_str(), 10, 1).y * 0.5f - 80;

    int text_two_pos_x = window.GetRendererCenter().x - MeasureTextEx(GetFontDefault(), splashscreen_text_2.c_str(), 10, 1).x * 0.5f;
    int text_two_pos_y = window.GetRendererCenter().y - MeasureTextEx(GetFontDefault(), splashscreen_text_2.c_str(), 10, 1).y * 0.5f + 90;
